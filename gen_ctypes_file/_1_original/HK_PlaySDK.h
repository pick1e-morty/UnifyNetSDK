...
#define BUF_VIDEO_SRC_EX                                                       \
  (7) // video render node count without hangnode;resetbuf need
...
#define PLAYM4_FEC_ERR_PLACETYPE                                               \
  117 // 安装方式不对.一个播放库port，对应一种安装方式
#define PLAYM4_FEC_ERR_CorrectType                                             \
  118 // 矫正方式错误：如矫正方式已有,则不能开多个；比如一个播放库port,设定除了PTZ和鱼眼半球矫正方式外,其他的矫正方式则只能开一路;设置180度矫正不能和ptz矫正一起开，半球矫正与其他矫正则无关联性。
...
typedef struct {
  unsigned char
      privt_type; // 用户扩展类型:详见IVS_PRIVT_INFO，10表示颜色+违禁品类型
...
typedef struct _VCA_TARGET_EX {
  ...
  VCA_RECT_F rect;           // 目标框
  unsigned char
      reserved1[40]; // 私有信息扩展字段，暂时以unsigned
                     // char数组表示，如果外部有需求再告知具体结构与信息
} VCA_TARGET_EX;

typedef struct _VCA_TARGET_LIST_EX {
  unsigned int target_num; // 目标数量
  VCA_TARGET_EX *
      pstTarget; // 目标内容，按照VCA_TARGET_EX的大小进行偏移读取各个目标具体的信息即可
} VCA_TARGET_LIST_EX;
...
typedef struct _VCA_TARGET_LIST_V1_EX_ {
  ...
  unsigned int target_num; // 目标数量
  HIK_TARGET_BLOB_EX *
      pstTarget; // 目标内容，按照HIK_TARGET_BLOB_EX的大小进行偏移读取各个目标具体的信息即可
...
typedef struct _VCA_RULE_LIST_V3_EX_ {
  ...
  VCA_RULE_EX *
      pstRule; // 规则内容，按照VCA_RULE_EX的大小进行偏移读取各个目标具体的信息即可
} VCA_RULE_LIST_V3_EX;
...
PLAYM4_API BOOL __stdcall PlayM4_SetFileRefCallBack(
    LONG nPort, void(__stdcall *pFileRefDone)(DWORD nPort, void *nUser),
    void *nUser);
...
PLAYM4_API BOOL __stdcall PlayM4_SetRefValue(LONG nPort, BYTE *pBuffer,
                                             DWORD nSize);
PLAYM4_API BOOL __stdcall PlayM4_GetRefValueEx(
    LONG nPort, BYTE *pBuffer,
    DWORD *pSize); ///< 与PlayM4_GetRefValueEx相比，增加索引帧类型和长度回调
...
PLAYM4_API BOOL __stdcall PlayM4_SetANRParam(LONG nPort, BOOL nEnable,
                                             int nANRLevel);
...
PLAYM4_API BOOL __stdcall PlayM4_SetCurrentFrameNum(LONG nPort,
                                                    DWORD nFrameNum);
PLAYM4_API
BOOL __stdcall PlayM4_GetSystemTime(LONG nPort,
...
PLAYM4_API BOOL __stdcall PlayM4_SetFileEndCallback(
    ...
    void *pUser);
PLAYM4_API BOOL __stdcall PlayM4_GetFileTotalTime(LONG nPort,
                                                  PLAYM4_SYSTEM_TIME *pstBegin,
                                                  PLAYM4_SYSTEM_TIME *pstStop);
///< 时区的表示以秒为单位，东区为正，西区为负
PLAYM4_API BOOL __stdcall PlayM4_SetSupplementaryTimeZone(int nPort,
                                                          int nTimeZone);
///< 若已经设置时区则返回设置时区，否则失败
PLAYM4_API BOOL __stdcall PlayM4_GetSupplementaryTimeZone(int nPort,
                                                          int *pTimeZone);
///< 获取时区信息
PLAYM4_API int __stdcall PlayM4_GetTimeZoneInfo(long nPort, int *pTimeZone);
// 获取封装和编码类型
PLAYM4_API int __stdcall PlayM4_GetStreamInfo(long nPort, int *pSysFormat,
                                              int *pCodeType);
///< 缓存操作
...
PLAYM4_API BOOL __stdcall PlayM4_SetSourceBufCallBack(
    ...
    void(CALLBACK *SourceBufCallBack)(long nPort, DWORD nBufSize, void *dwUser,
                                      void *pResvered),
    void *dwUser, void *pReserved);
...
PLAYM4_API BOOL __stdcall PlayM4_SetDecCallBackExMend(
    ...
    void(CALLBACK *DecCBFun)(long nPort, char *pBuf, long nSize,
                             FRAME_INFO *pFrameInfo, void *nUser,
                             void *nReserved2),
    char *pDest, long nDestSize, void *nUser);
...
PLAYM4_API BOOL __stdcall PlayM4_SetEncryptTypeCallBack(
    ...
    void(CALLBACK *EncryptTypeCBFun)(long nPort, ENCRYPT_INFO *pEncryptInfo,
                                     void *nUser, long nReserved2),
    void *nUser);

// 解码或显示回调中绝对时间戳（帧号）或相对时间戳（帧号）的回调设置;
//  nModule 表示回调模块选择，0 表示默认解码回调和显示回调
//  1：表示解码回调，2：表示显示回调； nType
//  表示回调绝对时间戳（帧号）还是相对时间戳（帧号）的类型设置，0：表示回调相对时间戳和相对帧号
//  1: 表示回调绝对时间戳和绝对帧号
// 显示回调是没有帧号回调的，所以显示回调下 0：表示回调相对时间戳 1:
// 表示回调绝对时间戳
PLAYM4_API BOOL __stdcall PlayM4_SetDecOrDisplayCallbackType(int nPort,
                                                             int nModule,
                                                             int nType);
...
PLAYM4_API BOOL __stdcall PlayM4_SetDisplayRegion(LONG nPort, DWORD nRegionNum,
                                                  ...
                                                  BOOL bEnable); ///< 电子放大
PLAYM4_API BOOL __stdcall PlayM4_SetDisplayRegionOnWnd(
    LONG nPort, DWORD nRegionNum, RECT *pSrcRect, BOOL bEnable); ///< 多窗口分割
...
PLAYM4_API BOOL __stdcall PlayM4_RegisterDrawFun(
    LONG nPort, void(CALLBACK *DrawFun)(long nPort, HDC hDc, void *nUser),
    void *nUser);
PLAYM4_API BOOL __stdcall PlayM4_SetEncTypeChangeCallBack(
    LONG nPort, void(CALLBACK *funEncChange)(long nPort, void *nUser),
    void *nUser);
...
PLAYM4_API BOOL __stdcall PlayM4_RenderPrivateData(LONG nPort, int nIntelType,
                                                   BOOL bTrue);
PLAYM4_API BOOL __stdcall PlayM4_RenderPrivateDataEx(LONG nPort, int nIntelType,
                                                     int nSubType, BOOL bTrue);
PLAYM4_API BOOL __stdcall PlayM4_SetOverlayPriInfoFlag(LONG nPort,
                                                       int nIntelType,
                                                       BOOL bTrue);
PLAYM4_API BOOL __stdcall PlayM4_SetTrackDurationTime(LONG nPort, int nTime);

// 私有信息回调接口，nType表示需要回调的私有信息类型，目前支持两种类型:
// 普通目标框：1; 深眸目标框； 2; 支持输入 1|2
// 的形式，即输入3表示两种类型都会回调
PLAYM4_API BOOL __stdcall PlayM4_RegisterIVSDrawFunCB_EX(
    long nPort,
    void(CALLBACK *IVSDrawFun)(long nPort, HDC hDC, FRAME_INFO *pFrameInfo,
                               INTEL_INFO_EX *pSyncData, void *dwUser),
    void *dwUser, unsigned int nType);

// lType: 1
// 表示获取当前显示帧PTZ信息。以特定结构体形式存储在pInfo内，plLen返回长度信息;起先传入pInfo
// = null，可以获取出需要分配的内存长度plLen
...
// 私有信息回调接口,nStamp表示相对时间戳信息；pSyncData表示私有信息内容,
// nType表示需要回调的私有信息类型，支持多个私有信息类型一起设置，
...
PLAYM4_API BOOL __stdcall PlayM4_RegisterIVSIntelInfoCB(
    LONG nPort,
    void(CALLBACK *IVSIntelInfoCBFun)(LONG nPort, HDC hDC, LONG nStamp,
                                      PRIVATE_INFO *pSyncData, void *dwUser),
    void *dwUser, DWORD nType, DWORD nMode);
...
PLAYM4_API BOOL __stdcall PlayM4_SetHLogFlag(int nPort, BOOL bFlag,
...
typedef struct _tagHDECODESUPPORT_ {
  ...
  unsigned char
      chDXVAH264_Max_Resolution; // DXVA,h264编码支持的最大的分辨率大概值-不是精准值-具体定义见枚举类型
  unsigned char
      chDXVAH265_Max_Resolution; // DXVA,h265编码支持的最大的分辨率大概值-不是精准值-具体定义见枚举类型
  unsigned char
      chCUVIDH264_Max_Resolution; // CUVID,h264编码支持的最大的分辨率大概值-不是精准值-具体定义见枚举类型
  unsigned char
      chCUVIDH265_Max_Resolution; // CUVID,h265编码支持的最大的分辨率大概值-不是精准值-具体定义见枚举类型
  unsigned char chValidFlag; ///< 有效
  unsigned char bD3D11VA;    // 判断是否支持D3D11VA硬解码:0-不支持，1-支持
  unsigned char
      chD3D11VAH264_Max_Resolution; // D3D11VA,h264编码支持的最大的分辨率大概值-不是精准值
  unsigned char
      chD3D11VAH265_Max_Resolution; // D3D11VA,h265编码支持的最大的分辨率大概值-不是精准值
...
typedef struct _tagD3D11_PIC_INFO_ {
  unsigned int
      nPicMode; // 抓图模式：0-按照码流实际分辨率抓图(之前抓图模式)；1-按照下面传入的宽高抓图（nPicWidth*nPicHeight为显示窗口宽高时效果最佳）
  unsigned char *pBuf; // 抓图数据buffer
  unsigned int
      nBufSize; // 缓存buffer大小-（如果nPicMode为0则为之前的根据获取到的码流分辨率来进行缓存申请；如果nPicMode为1则上层根据设置的分辨率来申请缓存）
  unsigned int *pPicSize; // 实际图片大小
  unsigned int
      nPicWidth; // 设置抓图宽-nPicMode为1时有效，且宽>=32,nPicWidth*nPicHeight<5000w分辨率。
  unsigned int
      nPicHeight; // 设置抓图高-nPicMode为1时有效，且高>=32,nPicWidth*nPicHeight<5000w分辨率。
...
typedef enum tagPLAYM4PostProcType {
  ...
  PLAYM4_PPT_HUE =
      0x2, ///< 色度   [0.0, 1.0]----0.166累加为一种颜色变化及0和1为同一种颜色
...
PLAYM4_API int __stdcall PlayM4_GetEngineSupport(
    long nPort, ENGINESUPPORT *pstEngineSupport);

///< 渲染库d3d11抓图-只支持d3d11渲染时有效，d3d9渲染无效:nType:0-jpeg,1-bmp.
PLAYM4_API int __stdcall PlayM4_GetD3DCapture(long nPort, unsigned int nType,
                                              D3D_PIC_INFO *pstPicInfo);

///< d3d11图像后处理-暂只支持d3d11渲染后处理-设置参数
PLAYM4_API int __stdcall PlayM4_SetD3DPostProcess(long nPort,
                                                  PLAYM4PostProcType nPostType,
                                                  float fValue);

///< d3d11图像后处理-暂只支持d3d11渲染后处理-获取参数
PLAYM4_API int __stdcall PlayM4_GetD3DPostProcess(long nPort,
                                                  PLAYM4PostProcType nPostType,
                                                  float *fValue);
...
typedef enum tagFECCorrectEffect {
  FEC_CORRECT_EFFECT_BACK_FACE_CULLING =
      0x100, // 背面剔除：非0表示启用，为0表示不启用，建议设置整数强转浮点数
...
PLAYM4_API BOOL __stdcall PlayM4_FEC_Rotate(
    LONG nPort,
    PLAYM4SRTRANSFERPARAM
        *pstRotateParam); ///< 此接口为不建议使用接口(老球面鱼眼旋转接口)
...
// 鱼眼抓图
PLAYM4_API BOOL __stdcall PlayM4_FEC_Capture(LONG nPort, unsigned int nSubPort,
                                             unsigned int nType,
                                             char *pFileName);
...
PLAYM4_API BOOL __stdcall PlayM4_FEC_GetCurrentPTZPort(LONG nPort,
                                                       float fPositionX,
                                                       float fPositionY,
                                                       unsigned int *pnPort);

PLAYM4_API BOOL __stdcall PlayM4_FEC_SetCurrentPTZPort(LONG nPort,
                                                       unsigned int nSubPort);

PLAYM4_API
BOOL __stdcall PlayM4_FEC_SetPTZOutLineShowMode(LONG nPort,
                                                FECSHOWMODE nPTZShowMode);

// 新的鱼眼半球矫正视角变化(旋转)两个相关接口
// 获取半球矫正视角参数(设置前先获取当前的值)
PLAYM4_API BOOL __stdcall PlayM4_FEC_GetViewParam(
    LONG nPort, unsigned int nSubPort, PLAYM4HRVIEWPARAMTYPE enViewParamType,
    float *fValue);
// 设置半球矫正视角变化参数(设置的值为获取值加上要偏移值)
PLAYM4_API BOOL __stdcall PlayM4_FEC_SetViewParam(
    LONG nPort, unsigned int nSubPort, PLAYM4HRVIEWPARAMTYPE enViewParamType,
    float fValue);
...
PLAYM4_API BOOL __stdcall PlayM4_FEC_SetDisplayRegion(
    LONG nPort, unsigned int nSubPort, unsigned int nType, DWORD nRegionNum,
    RECT *pSrcRect, HWND hDestWnd, BOOL bEnable);
// 背面剔除：非0表示启用，为0表示不启用，建议设置整数强转浮点数
PLAYM4_API BOOL __stdcall PlayM4_FEC_SetCorrectEffect(
    LONG nPort, unsigned int nSubPort, FECCORRECTEFFECT nCorrectEffect,
    float fValue);

// 设置3D模型参数-针对3D圆柱展开有效
PLAYM4_API
BOOL __stdcall PlayM4_FEC_Set3DModelParam(int nPort, unsigned int nSubPort,
                                          PLAYM4FEC3DMODELPARAM enType,
                                          float fValue);

// 获取特定视角参数 - 只适用于弧面，跟SetViewParam接口配合使用
PLAYM4_API BOOL __stdcall PlayM4_FEC_GetSpecialViewParam(
    int nPort, unsigned int nSubPort, PLAYM4FECSPECIALVIEWTYPE enSVType,
    PLAYM4HRVIEWPARAMTYPE enVPType, float *pValue);
...
typedef struct _PLAYM4_VIE_DYNPARAM_ {
  int moduFlag; // 启用的算法处理模块，在PLAYM4_VIE_MODULES中定义，如
                // PLAYM4_VIE_MODU_ADJ |
                // PLAYM4_VIE_MODU_EHAN，模块启用后，必须设置相应的参数；
  // PLAYM4_VIE_MODU_ADJ
...
// 设置参数，未开启模块的参数设置被忽略
PLAYM4_API
BOOL __stdcall PlayM4_VIE_SetParaConfig(LONG lPort,
                                        PLAYM4_VIE_PARACONFIG *pParaConfig);

// 获取开启模块的参数
PLAYM4_API
BOOL __stdcall PlayM4_VIE_GetParaConfig(LONG lPort,
                                        PLAYM4_VIE_PARACONFIG *pParaConfig);
...
PLAYM4_API BOOL __stdcall PlayM4_SetAdditionDataCallBack(
    LONG nPort, DWORD nSyncType,
    void(CALLBACK *AdditionDataCBFun)(long nPort,
...
PLAYM4_API BOOL __stdcall PlayM4_SetDecodeEngineEx(LONG nPort,
                                                   DWORD nDecodeEngine);
...
#define PLAYM4_RTINFO_ENCRYPT_ERROR                                            \
  5 // 秘钥错误 [加密的码流在解析报错时也会回调该错误码]
#define PLAYM4_RTINFO_RENDER_OVER 8 // 渲染一帧结束
#define PLAYM4_RTINFO_ERR_PRESENT                                              \
  16 // 渲染显示错误[当前渲染引擎渲染失败,上层重新切换引擎]
...
#define PLAYM4_RTINFO_IDMX_MEDIA_CHANGE_ERROR                                  \
  0x1000 // 媒体信息发生改变(与hik头的信息不一致或者psm里面的音视频类型发生改变)
#define PLAYM4_RTINFO_IDMX_DIFFERENT_FRAMERATE_ERROR                           \
  0x2000 // 封装层帧率与编码层帧率不一致
#define PLAYM4_RTINFO_IDMX_DIFFERENT_RESOLUTION_ERROR                          \
  0x4000 // 封装层分辨率与编码层分辨率不一致
#define PLAYM4_RTINFO_IDMX_DECORD_ERROR 0x8000     // 编码检查异常
#define PLAYM4_RTINFO_IDMX_PRIVT_LEN_ERROR 0x10000 // 私有数据长度异常
#define PLAYM4_RTINFO_SOFTDECODE_DATA_FLAKE_ERROR                              \
  0x20000 // 软解码数据存在花屏错误
...
PLAYM4_API BOOL __stdcall PlayM4_SetRunTimeInfoCallBackEx(
    LONG nPort, int nModule,
    void(CALLBACK *RunTimeInfoCBFun)(long nPort, RunTimeInfo *pstRunTimeInfo,
                                     void *pUser),
    void *pUser);

// 1、SetRunTimeInfoCallBackEx接口，nErrorCode类型6改为类型8；以后扩展方式
// 16、32、64，且从8开始消息默认关闭不发送。
// 2、新增实时消息控制接口，控制从8以后的消息，支持从8开始后消息，拼接模式
// 8|16|32 这种方式，上层主动选择接受类型。
// 3、nType可以通过或的方式将类型传入，nFlag表示清除（为0）或者进行回调（非0）；
PLAYM4_API int __stdcall PlayM4_SetRunTimeInfoCallbackType(int nPort,
                                                           int nModule,
                                                           unsigned int nType,
                                                           int nFlag);
...
typedef struct tagRenderModelMode {
  ...
  unsigned long
      ulDisplayType; // 按位组合 SR_MODEL_SOLID | SR_MODEL_FRAME | SR_MODEL_AXIS
  unsigned int
      nTransformMode; // 0 线框坐标轴随模型变换  1 线框坐标轴不随模型变换
...
PLAYM4_API BOOL __stdcall PlayM4_SR_SetConfig(LONG nPort, int nCfgType,
                                              void *pValue);

PLAYM4_API
BOOL __stdcall PlayM4_SR_Rotate(LONG nPort,
                                PLAYM4SRTRANSFERPARAM *pstRotateParam);

PLAYM4_API BOOL __stdcall PlayM4_SR_Capture(LONG nPort, unsigned int nType,
                                            char *pFileName);
...
PLAYM4_API BOOL __stdcall PlayM4_SetDecCallBack(
    LONG nPort, void(CALLBACK *DecCBFun)(long nPort, char *pBuf, long nSize,
                                         ...
                                         void *nReserved1, void *nReserved2));
PLAYM4_API BOOL __stdcall PlayM4_SetDecCallBackEx(
    LONG nPort,
    void(CALLBACK *DecCBFun)(long nPort, char *pBuf, long nSize,
...
PLAYM4_API
BOOL __stdcall PlayM4_SetSycStartTime(LONG nPort,
                                      PLAYM4_SYSTEM_TIME *pstSystemTime);
PLAYM4_API BOOL __stdcall PlayM4_SyncToAudio(LONG nPort, BOOL bSyncToAudio);
PLAYM4_API BOOL __stdcall PlayM4_SetDecodeEngine(LONG nPort,
                                                 DWORD nDecodeEngine);
PLAYM4_API BOOL __stdcall PlayM4_SetRunTimeInfoCallBack(
    LONG nPort,
    void(CALLBACK *RunTimeInfoCBFun)(long nPort, RunTimeInfo *pstRunTimeInfo,
                                     void *pUser),
    void *pUser);
...
// 预录像数据数据回调
PLAYM4_API BOOL __stdcall PlayM4_SetPreRecordCallBack(
    LONG lPort,
    void(CALLBACK *PreRecordCBfun)(long nPort,
...
PLAYM4_API BOOL __stdcall PlayM4_GetDDrawDeviceInfo(
    DWORD nDeviceNum, LPSTR lpDriverDescription, DWORD nDespLen,
    LPSTR lpDriverName, DWORD nNameLen, HMONITOR *hhMonitor);
...
PLAYM4_API BOOL __stdcall PlayM4_SetGlobalBaseTime(
    long nPort, PLAYM4_SYSTEM_TIME stGlobalBaseTime);
