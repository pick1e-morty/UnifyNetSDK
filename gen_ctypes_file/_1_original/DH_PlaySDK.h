 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetFileEndCallBack(LONG nPort,
                                                    fFileEndCBFun pFileEndCBFun,
                                                    void *pUserData);
 ...
 * @param[in] nBufPoolSize
 * 设置播放器中存放数据流的缓冲区大小。范围是SOURCE_BUF_MIN~SOURCE_BUF_MAX
 * @return BOOL，成功返回TRUE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_OpenStream(LONG nPort, PBYTE pFileHeadBuf,
 ...
 * @param[in] pUserData 用户数据
 */
typedef void(CALLBACK *pCallFunction)(LPBYTE pDataBuffer, DWORD DataLength,
                                      void *pUserData);
...
typedef enum {
  ...
  AUDIO_AAC_ENC,     // AAC编码带私有封装头, 采样率支持:
                     // 8,11.025,12,16,22.05,24,32,44.1,48,64KHz
  AUDIO_AMR_NB_ENC,  // AMR窄带编码带私有封装头(仅支持8000采样率)
  AUDIO_CAPTURE_END,
} AUDIO_CAPTURE_TYPE;
PLAYSDK_API BOOL CALLMETHOD PLAY_OpenAudioRecord(pCallFunction pProc,
                                                 LONG nBitsPerSample,
                                                 LONG nSamplesPerSec,
                                                 LONG nLength, LONG encodetype,
                                                 void *pUserData);
 ...
 * @param[in] scEffect
 变声效果，有效值0-3，0表示原声，1表示成年人变大叔音，2表示小孩变大叔音，3表示电子音
 * @param[in] scTsm
 变声效果尺度因子,默认值为0.0，当tms是0.0,则使用sc_effect默认的tsm值，否则使用设置的tsm值
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SpeechChange(BOOL bStart, int scEffect,
                                              float scTsm);
 ...
 *
 * @param[in] fRatio
 * 缩放比例。大于0小于1为音频缩小；1为原始音频；大于1为音频放大
 * @return BOOL，成功返回TURE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetAudioRenderScaling(LONG nPort,
                                                       float fRatio);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetAudioRenderScaling(LONG nPort,
                                                       float *pfRatio);
 ...
 *
 * @param[in] nPort 通道号
 * @param[in] nCoefficient
 * 调整参数，0是不调整，其他取值范围从MIN_WAVE_COEF到MAX_WAVE_COEF。若设定为MIN_WAVE_COEF表
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetColor(LONG nPort, DWORD nRegionNum,
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetColor(LONG nPort, DWORD nRegionNum,
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetAVSyncType(LONG nPort,
                                               AV_SYNC_TYPE nAVSyncType);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetPlayDirection(LONG nPort,
                                                  DWORD emDirection);
 ...
 * 阈值时间。缓冲小于该时间正常播放，大于该时间稍微快放
 * @return 成功返回TURE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetDelayTime(LONG nPort, int nDelay,
                                              int nThreshold);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetPlayMethod(LONG nPort, int nStartTime,
                                               int nSlowTime, int nFastTime,
                                               int nFailedTime);
 ...
 *
 * @param[in] nPort 通道号
 * @param[in] nrotateType
 * 翻转类型。0：不旋转；1：旋转90度；2：旋转180度；3：旋转270度
 * @return 成功返回TURE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetCurrentFrameNum(LONG nPort,
                                                    DWORD nFrameNum);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetFileRefCallBack(
    LONG nPort, fFileRefDoneCBFun pFileRefDoneCBFunc, void *pUserData);
/**
 * 建立索引回调函数。
 *
 * @param[in] nPort 通道号
 ...
 * @param[in] pUserData 用户自定义参数
 */
typedef void(CALLBACK *fFileRefDoneCBFunEx)(DWORD nPort, BOOL bIndexCreated,
                                            void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetFileRefCallBackEx(
    LONG nPort, fFileRefDoneCBFunEx pFileRefDoneCBFunEx, void *pUserData);
 ...
 * @param[in] nType
 * 指定nValue的类型。如果nType是BY_FRAMENUM，则nValue表示帧号；nType是BY_FRAMETIME，则nValue表示时间，单位ms。
 * @param[in] pFramePos 查找到的关键帧的文件位置信息结构指针，详见PFRAME_POS。
 * @return BOOL，成功返回TRUE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetKeyFramePos(LONG nPort, DWORD nValue,
                                                DWORD nType,
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetNextKeyFramePos(LONG nPort, DWORD nValue,
                                                    DWORD nType,
                                                    PFRAME_POS pFramePos);
 ...
 * @param[in/out] pSize
 * 输入pBuffer的大小，输出索引信息大小。可以在第一次指定pSize=0，pBuffer=NULL，
 *                  从pSize的返回值获得需要的缓冲区大小。然后分配足够的缓冲，再调用一次。
 * @return BOOL，成功返回TRUE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetRefValue(LONG nPort, BYTE *pBuffer,
                                             DWORD *pSize);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetRefValue(LONG nPort, BYTE *pBuffer,
                                             DWORD nSize);
 ...
 */
typedef void(CALLBACK *fCBDecode)(LONG nPort,
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetDecodeCallBack(LONG nPort, fCBDecode cbDec,
                                                   void *pUser);
 ...
 */
typedef void(CALLBACK *fDisplayCBFun)(LONG nPort, char *pBuf, LONG nSize,
                                      LONG nWidth, LONG nHeight, LONG nStamp,
                                      LONG nType, void *pReserved);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetDisplayCallBack(LONG nPort,
                                                    fDisplayCBFun DisplayCBFun,
                                                    void *pUserData);
 ...
 */
typedef void(CALLBACK *fAudioCBFun)(LONG nPort, char *pAudioBuf, LONG nSize,
                                    LONG nStamp, LONG nType, void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetAudioCallBack(LONG nPort,
                                                  fAudioCBFun AudioCBFun,
                                                  void *pUserData);
...
PLAYSDK_API BOOL CALLMETHOD PLAY_SetVisibleDecodeCallBack(
    LONG nPort, fVisibleDecodeCallBackFunc cbDec, void *pUser);
 ...
 */
typedef void(CALLBACK *fDecCBFun)(LONG nPort, char *pBuf, LONG nSize,
                                  FRAME_INFO *pFrameInfo, void *pUserData,
                                  LONG nReserved2);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetDecCallBackEx(LONG nPort,
                                                  fDecCBFun DecCBFun,
                                                  void *pUserData);
 ...
 */
typedef void(CALLBACK *fVisibleDecCBFun)(LONG nPort, char *pBuf, LONG nSize,
                                         FRAME_INFO *pFrameInfo,
                                         void *pUserData, LONG nReserved1);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetVisibleDecCallBack(
    LONG nPort, fVisibleDecCBFun DecCBFun, void *pUserData);
 ...
 * @param[in] reallen 缓冲的实际长度
 * @param[in] type
 * 1没有错误；2水印错误；3帧数据错误；4帧号错误；5帧头错误；6没有帧头
 * @param[in] pUserData 用户自定义参数
 * @return int 无意义
 */
typedef int(CALLBACK *fGetWaterMarkInfoCallbackFunc)(char *buf, LONG key,
                                                     LONG len, LONG reallen,
                                                     LONG type,
                                                     void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetWaterMarkCallBack(
    LONG nPort, fGetWaterMarkInfoCallbackFunc pFunc, void *pUserData);
 ...
 * @param[in] lInfoType
 * 水印信息类型,有三种类型,WATERMARK_DATA_TEXT,WATERMARK_DATA_JPEG_BMP,WATERMARK_DATA_FRAMEDATA
 * @param[in] len 缓冲的最大长度
 * @param[in] reallen 缓冲的实际长度
 * @param[in] lCheckResult
 * 1没有错误；2水印错误；3帧数据错误；4帧号错误；5帧头错误；6没有帧头
 * @param[in] pUserData 用户自定义参数
 * @return int 无意义
 */
typedef int(CALLBACK *fGetWaterMarkInfoCallbackFuncEx)(
    LONG nPort, char *buf, LONG lTimeStamp, LONG lInfoType, LONG len,
    LONG reallen, LONG lCheckResult, void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetWaterMarkCallBackEx(
    LONG nPort, fGetWaterMarkInfoCallbackFuncEx pFunc, void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetEncTypeChangeCallBack(
    LONG nPort, fEncChangeCBFun EncChangeCBFun, void *pUserData);
 ...
 */
typedef void(CALLBACK *fEncChangeCBFunEx)(LONG nPort, void *pUserData,
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetEncTypeChangeCallBackEx(
    LONG nPort, fEncChangeCBFunEx EncChangeCBFun, void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_QueryInfo(LONG nPort, int cmdType, char *buf,
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetRealFrameBitRate(LONG nPort,
                                                     double *pBitRate);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetPictureSize(LONG nPort, LONG *pWidth,
                                                LONG *pHeight);
...
typedef struct {
  ...
  char
      nCompFrameVerifyStatus; // 数据完整校验标识，0:默认不开启，不校验；1：其他类型错误，导致未校验；2：经过校验，校验结果失败；3：经过校验，校验结果成功；
...
typedef void(CALLBACK *fDemuxDecCBFun)(LONG nPort, char *pBuf, LONG nSize,
                                       void *pMutexInfo, void *pMutexInfoEx,
                                       void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetDemuxCallBack(LONG nPort,
                                                  fDemuxDecCBFun DecCBFun,
                                                  void *pUserData);

#if defined WEBWASM

extern void cPlusVisibleDecCallBack(int nPort, char *pBufY, char *pBufU,
                                    char *pBufV, int nSize,
                                    FRAME_INFO_EX *pFrameInfo);

extern void cDigitalSignCallBack(int nPort, int nFrameID, BOOL bSuccess);

extern void cRecordDataCallBack(int nPort, FRAME_INFO_EX *pFrameInfo,
                                unsigned char *pData, int nDataSize,
                                long long nOffset);

extern void cIVSDrawDataCallBack(int nPort, char *buf, int type, int len,
                                 int reallen);
 ...
 * @param[in] pUserData 用户自定义参数
 */
typedef int(CALLBACK *fH264InfoCBFun)(LONG nPort, char *pBuf, LONG nLen,
                                      void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_Set264EncodeInfoCallBack(
    LONG nPort, fH264InfoCBFun pH264InfoCBFun, void *nUser);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetSecurityKey(LONG nPort, const char *szKey,
                                                DWORD nKeylen);
...
typedef enum {
  ...
  ENCRYPT_SM4_OFB,
  /*国密加密算法SM4_OFB*/ // 不知道具尸是哪种国密加密，就填这种SP_ENCRYPT_SM4_OFB
} ENCRYPT_TYPE;
...
typedef struct {
  ...
  int KeyLen;
  char *KeyId; // AES:不用填； AES256(GDPR一期):keyid；
               // AES256(GDPR二期)和国密:vkekid ；
  int KeyIdLen;
  ...
  char
      nKeyType; // 用于区分pKey代表的类型，仅适用于加密辅助帧存在时用户下发的秘钥类型，0：pKey代表vkek，用于解密evek的秘钥；1：pKey代表解密码流vk；当码流不存在加密辅助帧，此变量无意义
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetSecurityKeyEx(LONG nPort,
                                                  ENCRYPT_TYPE nType,
                                                  DECRYPT_PARAM *key,
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetEncryptKey(LONG nPort,
                                               ENCRYPT_TYPE nEncryptType,
                                               char *szKey,
 ...
 * @param[in] nPort 通道号
 * @param[in] nRegionNum
 * 显示区域序号,0~(MAX_DISPLAY_WND-1),如果为0,则将设置的区域显示在主窗口中
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetDisplayRegion(LONG nPort, DWORD nRegionNum,
                                                  DISPLAYRECT *pSrcRect,
 ...
 * @return
 *DWORD，根据不同参数返回缓冲区值，源缓冲区返回byte，解码后缓冲区返回帧数
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
 ...
 * @param[in] pUserData 用户自定义参数
 * @note 录制回调，当nPercent为100时,
 * 需要在回调线程中同步地停止录制，以防止不能及时停止进而导致录制数据比预期大
 */
typedef void(CALLBACK *fPercentCallbackFunc)(LONG nPort, int nPercent,
                                             void *pUserData);

/**
 * 进度回调。可以设定文件开始结束时间，如果指定，需要PLAY_SetFileRefCallBack回调后才有效，[0,0]
 * 表示整个文件大小，不需要建立索引，在PLAY_OpenFile之后调用。
 *
 * @param[in] nPort 通道号
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD
PLAY_SetPercentCallBack(LONG nPort, LONG nStartTime, LONG nEndTime,
                        fPercentCallbackFunc pFunCallback, void *pUserData);
 ...
 * @param[in/out] pmax_ret_num
 * 输入查询的最大文件个数，返回查询到的文件个数，文件系统最多一次返回pmax_ret_num传入的条数
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 * 文件名字参考:
 * "/part/uuid/%4d-%02d-%02d/%03d/%02d.%02d.%02d-%02d.%02d.%02d/cType/first_clus_no.dav"
 */
PLAYSDK_API UINT CALLMETHOD PLAY_QueryFileList(
    UINT nChannel, USER_TIME start_time, USER_TIME end_time, UINT *pmax_ret_num,
    pFILE_INFO pfile_info, UINT type, BYTE nDiskIndex, UINT driver_type);
...
typedef struct {
  FISHEYE_MOUNTMODE
  subMountMode; // 子图像安装模式, 仅当图像主校正模式为用户自定义模式时,
                // 该值有效
  FISHEYE_CALIBRATMODE
  subCalibrateMode; // 子图像校正模式, 仅当图像主校正模式为用户自定义模式时,
                    // 该值有效
...
typedef struct {
  FISHEYE_SIZE
  mainShowSize; // 主显示窗口分辨率,
                // 算法据此输出适合的最优结果(不变形情况下尽量达到该比例)
  FISHEYE_SIZE
  floatMainShowSize; // 输出双buffer时使用，目前暂时还是用老的操作方法，浮动窗口的主显示窗口分辨率,
                     // 浮动圆的宽高比需要为1:1，浮动壁装全景的宽高比需要为16:9
  FISHEYE_SIZE imgOutput; // 输出图像分辨率(缩放前),
                          // 图像主校正模式为用户自定义模式时为外部输入,
                          // 其他模式为内部返回
  FISHEYE_SUBMODE
  *subMode;       // 子模式信息, 图像主校正模式为用户自定义模式时为外部输入,
                  // 其他模式为内部返回
  int subModeNum; // 子模式数, 图像主校正模式为用户自定义模式时为外部输入,
                  // 其他模式为内部返回
  int outputSizeRatio; // 暂不启用, 校正输出图像的缩放比,Q8格式,范围0-256,
                       // 256为保持最大输出分辨率
  int reserved[1];     // 保留字节
...
typedef struct {
  FISHEYE_SIZE
  mainStreamSize; // 对应主码流原始宽高，当传入分辨率与之不等时表明为辅码流是此分辨率缩放而来
  ...
  FISHEYE_MODEINITPARAM
  modeInitParam; // 外部传入模式初始化各画面信息，适用于模式切换恢复到上一次的状态,
  FISHEYE_OUTPUTFORMAT *outputFormat; // 输出图像信息
  MHFPTZ_CONFIGPARAM *configParam;    // 鱼球联动配置参数
  int enableAutoContrast; // 开启自动对比度, 0关闭, 1开启, 该功能会增加算法耗时,
                          // 需要性能好的机器才建议开启
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_OptFisheyeParams(
    LONG nPort, FISHEYE_OPERATETYPE operatetype, FISHEYE_OPTPARAM *pOptParam);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_FisheyeSecondRegion(
    LONG nPort, HWND hDestWnd, FISHEYE_OPTPARAM *pOptParam, BOOL bEnable);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_FisheyeEptzUpdate(
    LONG nPort, FISHEYE_EPTZPARAM *pEptzParam, BOOL bSecondRegion);
 ...
 * @param[in] pUserData 用户自定义参数
 */
typedef void(CALLBACK *fFishEyeInfoFun)(LONG nPort, BYTE byCorrectMode,
                                        WORD wRadius, WORD wCircleX,
                                        WORD wCircleY, UINT widthRatio,
                                        UINT heightRatio, BYTE gain,
                                        BYTE denoiseLevel, BYTE installStyle,
                                        void *pUserData);
...
PLAYSDK_API BOOL CALLMETHOD PLAY_SetFishEyeInfoCallBack(
    LONG nPort, fFishEyeInfoFun pFishEyeInfoFun, void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_CatchPicEx(LONG nPort, char *sFileName,
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_ConvertToBmpFile(char *pBuf, LONG nSize,
                                                  LONG nWidth, LONG nHeight,
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_ConvertToJpegFile(char *pYUVBuf, LONG nWidth,
                                                   LONG nHeight, int YUVtype,
                                                   int quality,
                                                   char *sFileName);
...
PLAYSDK_API BOOL CALLMETHOD
PLAY_ConvertToImageData(ImageConvertInfo *pImageInfo);

/**
 * 抓取BMP图像。
 *
 * @param[in] nPort 通道号
 * @param[in] pBmpBuf 存放BMP图像数据的缓冲地址，由用户分配，不得小于bmp
 * 图像大小，
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetPicBMP(LONG nPort, PBYTE pBmpBuf,
                                           DWORD dwBufSize, DWORD *pBmpSize);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetPicBMPEx(LONG nPort, PBYTE pBmpBuf,
                                             DWORD dwBufSize, DWORD *pBmpSize,
                                             LONG nWidth, LONG nHeight,
                                             int nRgbType);
 ...
 * @param[in] nPort 通道号
 * @param[in] pJpegBuf
 存放JPEG图像数据的缓冲地址，由用户分配，不得小于JPEG图像大小，
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetPicJPEG(LONG nPort, PBYTE pJpegBuf,
                                            DWORD dwBufSize, DWORD *pJpegSize,
                                            int quality);
 ...
 * @param[in] nPort 通道号
 * @param[in] pTiffBuf
 存放TIFF图像数据的缓冲地址，由用户分配，不得小于tiff图像大小，
   推荐大小：w*h*3+1024，
   其中w和h分别为图像宽高
 * @param[in] dwBufSize 缓冲区大小
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetPicTIFF(LONG nPort, PBYTE pTiffBuf,
                                            DWORD dwBufSize, DWORD *pTiffSize);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_CatchResizePic(LONG nPort, char *sFileName,
                                                LONG lTargetWidth,
                                                LONG lTargetHeight,
                                                tPicFormats ePicfomat);
...
/*
 * 注册一个回调函数，获得当前表面的device
 * context，你可以在这个DC上画图(或文字)，
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_RigisterDrawFun(LONG nPort,
                                                 fDrawCBFun DrawCBFun,
                                                 void *pUserData);
 ...
 * @param[in] pUserData 用户自定义参数
 */
typedef void(CALLBACK *fDrawCBRenderHandleFun)(LONG nPort, int nReginNum,
                                               void *pRenderHandle,
                                               void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_RigisterDrawRendleHandleFun(
    LONG nPort, fDrawCBRenderHandleFun DrawCBFun, void *pUserData);
 ...
 * @param[in] nPort 通道号
 * @param[in] nReginNum
 *显示区域序号，范围[0,(MAX_DISPLAY_WND-1)]。如果nRegionNum为0，
 *   则将设置的区域显示在主窗口中。
 * @param[in] hDc OffScreen表面设备上下文
 * @param[in] pUserData 用户自定义参数
 */
typedef void(CALLBACK *fDrawCBFunEx)(LONG nPort, LONG nReginNum, HDC hDc,
                                     void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_RigisterDrawFunEx(LONG nPort, LONG nReginNum,
                                                   fDrawCBFunEx DrawFunEx,
                                                   void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_StartPrepareRecord(LONG nPort,
 ...
 *
 * @param[in] nPort 通道号
 * @param[in] lMediaChangeType
 * AVI_MEDIACHANGE_FRAMERATE表示帧率改变；AVI_MEDIACHANGE_RESOLUTION表示分辨率改变
 * @param[in] pUserData 用户自定义参数
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
typedef void(CALLBACK *fAVIConvertCallback)(LONG nPort, LONG lMediaChangeType,
                                            void *pUserData, BOOL *pIsNewFile,
                                            char *sNewFileName);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_StartAVIConvert(LONG nPort, char *sFileName,
                                                 fAVIConvertCallback pAVIFunc,
                                                 void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetEngine(LONG nPort, DecodeType decodeType,
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_GetPictureQuality(LONG nPort,
                                                   BOOL *bHighQuality);
...
typedef enum _PLAY_STRATEGE {
  ...
  PLAY_THROW_FRAME_FLAG_HIGHT =
      1, // 默认策略抽帧，大于1080P码流4倍数(含)以上抽帧，[1080P,720P]码流8倍数(含)以上抽帧，小于720P码流16倍数(含)以上抽帧
...
PLAYSDK_API BOOL CALLMETHOD PLAY_EnableLargePicAdjustment(LONG nPort,
                                                          int bEnable);
 ...
 *				 0 :
 *表示解码中遇到码流数据不符合协议继续解码，错误码流会花屏
 * @return BOOL，成功返回TRUE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetDecodeStrategy(LONG nPort,
                                                   int nStrategyType);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_RenderPrivateData(LONG nPort, BOOL bTrue,
                                                   LONG nReserve);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetIvsEnable(LONG nPort, int nIvsType,
                                              BOOL bEnable);
 ...
 */
PLAYSDK_API BOOL CALLMETHOD
PLAY_SetIVSTrackEx2Config(int nPort, IVSDRAWER_TrackEx2Config trackex2config);
 ...
 * @param[in] pUserData  用户自定义数据
 */
typedef void(CALLBACK *fDataCBFun)(LONG nPort, char *pBuf, LONG nSize,
                                   void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetDataCallBack(LONG nPort,
                                                 fDataCBFun pDataCBFun,
                                                 void *pUserData);
 ...
 *
 * @param[in] pIVSBuf
 *辅助帧数据(json或智能帧结构体数据)，当为智能帧结构体数据时，pIVSBuf为IVS
 *Object结构体数组的起始地址
 * @param[in] nIVSType 辅助帧数据类型,详见IVS_TYPE
 *			取值为IVSINFOTYPE_RAWDATA时，对应原始json数据
 *			取值为IVSINFOTYPE_TRACK时，单个IVS object对应结构体
 *SP_IVS_OBJ_EX； 取值为IVSINFOTYPE_TRACK_EX_B0时，单个IVS object对应结构体
 *SP_IVS_COMMON_OBJ；
 * @param[in] nIVSBufLen 辅助帧数据长度(json或智能帧结构体数据)，
 *			  当为智能帧结构体数据时，nIVSBufLen为IVS
 *Object的个数乘以单个IVS object的长度，单个IVS object的长度可由type备注类型获得
...
typedef void(CALLMETHOD *fIVSInfoCallbackFunc)(char *pIVSBuf, LONG nIVSType,
                                               LONG nIVSBufLen, LONG nFrameSeq,
                                               void *pReserved,
                                               void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetIVSCallBack(LONG nPort,
                                                fIVSInfoCallbackFunc pFunc,
                                                void *pUserData);
 ...
 * @return int 无意义
 */
typedef int(CALLBACK *fGPSInfoCallbackFunc)(char *pBuf, LONG nLen,
                                            void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetGPSCallBack(LONG nPort,
                                                fGPSInfoCallbackFunc pFunc,
                                                void *pUserData);
 ...
 * @return int 无意义
 */
typedef int(CALLBACK *fStatisticCallbackFunc)(PSTATISTIC_INFO pStatisticData,
                                              void *pUserData);
 ...
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetStatisticCallBack(
    LONG nPort, fStatisticCallbackFunc pFunc, void *pUserData);
 ...
 * @return BOOL，成功返回TRUE，失败返回FALSE
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetViewProportion(LONG nPort,
                                                   int nWidthProportion,
                                                   int nHeightProportion);
 ...
 */
PLAYSDK_API BOOL CALLMETHOD
PLAY_SetAudioPlaybackMode(LONG nPort, AUDIO_PLAYBACK_MODE nMode);
 ...
 * @return BOOL,成功返回TRUE，失败返回FALSE
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_ViewResolutionChanged(LONG nPort, int nWidth,
                                                       int nHeight,
                                                       DWORD nRegionNum);
...
typedef struct profiled_window_mask {
  char *pMaskData;     // 遮罩位图数据，R8格式，每位数值范围为[0,255]
                       // 255是全不透明，0为全透明
 ...
 * @return BOOL，成功返回TRUE，失败返回FALSE
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_DrawProfiledWindow(LONG nPort, BOOL bEnable,
                                                    ProfieldWindowMask *pMask);
 ...
 * @param[in] MutiTargetTactic 多目标情况下的跟踪策略
 * 0：继续跟踪，1：回正，不继续跟踪
 * @return BOOL，成功返回TRUE，失败返回FALSE
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_EnableAutoTrack(LONG nPort, BOOL bEnable,
                                                 int MutiTargetTactic);
// 消息回调，通知当前是否处于跟踪状态
typedef void(CALLMETHOD *fAutoTrackInfoCallbackFunc)(BOOL bEnable,
                                                     void *pUserData);
/**
 * @brif 自动跟踪配置选项结构体
 */
 ...
 * @return BOOL，成功返回TRUE，失败返回FALSE
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetMutiWindowAutoTrack(
    LONG nPort, AutoTrackConfig *pAutoTrackConfig, BOOL bEnable);
 ...
 *
 * @param[in] nBufPoolSize
 * 置播放器中存放数据流的缓冲区大小，范围是[SOURCE_BUF_MIN,SOURCE_BUF_MAX]
 * @return DWORD,失败返回0，否则返回通道号
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
 ...
 * @param[in] nMsg
 * 用户输入的消息，当解码时编码格式发生改变时用户在hWnd窗口过程中收到这个消息
 * @return BOOL，成功返回TRUE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetEncChangeMsg(LONG nPort, HWND hWnd,
                                                 UINT nMsg);
 ...
 * @param[in] nMsg
 * 用户自定义的输入的消息，当播放到文件结束时用户在hWnd窗口过程中
 * @return BOOL，成功返回TRUE，失败返回FALSE
 * @note 如果返回失败，可以调用PLAY_GetLastErrorEx接口获取错误码。
 */
PLAYSDK_API BOOL CALLMETHOD PLAY_SetFileEndMsg(LONG nPort, HWND hWnd,
                                               UINT nMsg);
