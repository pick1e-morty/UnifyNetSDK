...
AppendErrorInfo = {  # 因为上面那个ErrorCode字典是很容易就拿到的，不能为了几个小小的错误信息的追加操作就改变上面那个字典的结构。暂时不值得
    1079: "错误码索引重复了，该错误码所对应的另一个类型为NET_ERROR_FACE_RECOGNITION_SERVER_DELETE_GROUP_ERROR",  # 两个不同的库错误代码撞了
    1080: "错误码索引重复了，该错误码所对应的另一个类型为NET_ERROR_FACE_RECOGNITION_SERVER_NAME_FORMAT_ERROR",  # dahua你真行啊
    1081: "错误码索引重复了，该错误码所对应的另一个类型为NET_ERROR_FACE_RECOGNITION_SERVER_FILEPATH_NOT_SET",
}
