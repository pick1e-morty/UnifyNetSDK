...
class DaHuaNetSDK(AbsNetSDK):
    ...
    def setArgTypes_ResType(self):
        ...
        self.netDll.CLIENT_LoginWithHighLevelSecurity.argtypes = [
            POINTER(DH.NET_IN_LOGIN_WITH_HIGHLEVEL_SECURITY),
            POINTER(DH.NET_OUT_LOGIN_WITH_HIGHLEVEL_SECURITY),
        ]
        self.netDll.CLIENT_LoginWithHighLevelSecurity.restype = c_longlong
        self.netDll.CLIENT_QueryRecordFile.argtypes = [
            c_longlong,
            c_int,
            c_int,
            DH.LPNET_TIME,
        ...
        self.netDll.CLIENT_DownloadByTimeEx.argtypes = [
            c_longlong,
            c_int,
            c_int,
            DH.LPNET_TIME,
        ...
        self.netDll.CLIENT_GetDownloadPos.argtypes = [
            c_longlong,
            POINTER(c_int),
            POINTER(c_int),
        ]
        ...
        self.netDll.CLIENT_RealPlayEx.argtypes = [
            c_longlong,
            c_int,
            POINTER(None),
            DH.DH_RealPlayType,
        ...
        self.netDll.CLIENT_PlayBackByTimeEx.argtypes = [
            c_longlong,
            c_int,
            DH.LPNET_TIME,
            DH.LPNET_TIME,
    ...
    @classmethod
    def init(cls) -> bool:
        initResult = cls.netDll.CLIENT_InitEx(
            DH.fDisConnect(0), 0, DH.NETSDK_INIT_PARAM()
        )
    ...
    def getTime_CFG(cls, userID: int) -> datetime:
        ...
        waittime = 1000
        result = cls.netDll.CLIENT_GetDevConfig(
            userID,
            DH.DH_DEV_TIMECFG,
            lChannel,
            byref(lpOutBuffer),
    ...
    def setTime_CFG(cls, userID: int, pyTime: datetime) -> bool:
        ...
        waittime = 1000
        result = cls.netDll.CLIENT_SetDevConfig(
            userID,
            DH.DH_DEV_TIMECFG,
            lChannel,
            byref(lpOutBuffer),
    ...
    def getNTP_CFG(cls, userID: int) -> UninfyNTPArg:
        ...
        if lpOutBuffer.nType == 0:
            ntpArg.domainOrIP = (
                t_szHostIp  # 这个szHostIp可能零几年的遗留参数吧，现在没被用了
            )
    ...
    def setNTP_CFG(cls, userID: int, ntpArg: UninfyNTPArg) -> bool:
        ...
        lpInBuffer.nTimeZone = (
            dongBaTimeZone if ntpArg.timeZone is None else ntpArg.timeZone
        )  # 用户不传时间区，默认东八区
        ...
        logger.debug(
            f"设置NTP参数后，需要再次获取NTP参数才能在前端中看到结果，结果为{MustRuningResult}"
        )
    ...
    def realPlay(cls, userID: int, channel: int, hwnd) -> int:
        ...
        realPlayHandle = cls.netDll.CLIENT_RealPlayEx(
            userID, channel, valid_hwnd, DH.DH_RType_Multiplay_4
        )
    ...
    def playBackByTime(cls, userID: int, playBackArg: UnifyPlayBackByTimeArg) -> int:
        ...
        downloadPosCallBack = (
            DH.fDownLoadPosCallBack(0)
            if playBackArg.downLoadPosCallBack is None
            else playBackArg.downLoadPosCallBack
        )
        dwPosUser = 0 if playBackArg.dwPosUser is None else playBackArg.dwPosUser
        dataCallBack = (
            DH.fDataCallBack(0)
            if playBackArg.dataCallBack is None
            else playBackArg.dataCallBack
        )
        dwDataUser = 0 if playBackArg.dwDataUser is None else playBackArg.dwDataUser
        playBackHandle = cls.netDll.CLIENT_PlayBackByTimeEx(
            userID,
            channel,
            byref(startDateTime),
            byref(endDateTime),
    ...
    def catchPicture(cls, lPlayHandle: int, savedFileName: str) -> bool:
        ...
        catchResult = cls.netDll.CLIENT_CapturePictureEx(
            lPlayHandle, savedFileName, DH.NET_CAPTURE_JPEG_70
        )
    ...
    @classmethod
    def login(
        cls, loginArg: UnifyLoginArg
    ) -> (int, DH.NET_OUT_LOGIN_WITH_HIGHLEVEL_SECURITY):
        ...
        login_id = cls.netDll.CLIENT_LoginWithHighLevelSecurity(
            byref(loginInfo), byref(deviceInfo)
        )
        cls.getLastError("CLIENT_LoginWithHighLevelSecurity", bool(login_id))
        return login_id, deviceInfo

    @classmethod
    def syncFindFileByTime(
        cls, userID: int, findFileArg: UnifyFindFileByTimeArg
    ) -> (
        bool
    ):  # 大华的sdk，能通过时间查询的就这一个方法，且没有返回给我查找句柄，所以大华这边只能有sync了
        # 同步阻塞 按照时间查找录像文件是否存在
        return cls.__findFileByTime(
            userID, findFileArg
        )  # CLIENT_FindFileEx能返回句柄，但不能按照时间查询
    ...
    def __findFileByTime(cls, userID: int, findFileArg: UnifyFindFileByTimeArg) -> bool:
        ...
        waittime = 2000
        byTime = True
        findFileResult = cls.netDll.CLIENT_QueryRecordFile(
            userID,
            nChannelId,
            nRecordFileType,
            byref(tmStart),
    ...
    def stopDownLoadTimer(cls, downLoadHandle: int) -> bool:
        ...
        getDownLoadPosResult = cls.netDll.CLIENT_GetDownloadPos(
            downLoadHandle, byref(nTotalSize), byref(nDownLoadSize)
        )
        cls.getLastError("CLIENT_GetDownloadPos", bool(getDownLoadPosResult))
        logger.trace(
            f"下载ID {downLoadHandle} ,下载总长度 {nTotalSize.value}KB ,已下载长度 {nDownLoadSize.value}KB "
        )
    ...
    @classmethod
    def syncDownLoadByTime(
        cls, userID: int, downLoadArg: UnifyDownLoadByTimeArg
    ) -> bool:
        # 同步阻塞，按照时间参数下载录像
        downLoadHandle = cls.__downLoadByTime(userID, downLoadArg)
        while True:
            downLoadResult = cls.stopDownLoadTimer(
                downLoadHandle
            )  # 每0.5秒查一下有没有下载完成
    ...
    def __downLoadByTime(cls, userID: int, downLoadArg: UnifyDownLoadByTimeArg) -> int:
        ...
        savedFileName = create_string_buffer(
            str(downLoadArg.saveFilePath).encode("gbk")
        )
        ...
        downLoadHandle = cls.netDll.CLIENT_DownloadByTimeEx(
            userID,
            channel,
            DH.EM_RECORD_TYPE_ALL,
            startDateTime,
            endDateTime,
            savedFileName,
            DH.fTimeDownLoadPosCallBack(0),
            0,
            DH.fDataCallBack(0),
            0,
            pReserved,
