...
if __name__ == "__main__":
    os.chdir(r"../headfile/lib/win")  # 加载库,先加载依赖库
    ...
    resCode = Objdll.NET_DVR_GetDVRConfig(
        lUserId,
    ...
    if resCode == 0:
        print(
            "NET_DVR_GET_IPPARACFG_V40 fail, error code is: %d"
    ...
    for i in range(
        起始模拟通道, 设备模拟通道数量 + 1
    ):  # ipcfg.byAnalogChanEnable 模拟通道数量在这个数组里，1表示有通道，0表示没有通道，不过我司用的应该都是32通道的
        ...
        if resCode == 1:
            ...
            print(
                "NET_DVR_GET_PICCFG_V30 fail, error code is: %d"
                % Objdll.NET_DVR_GetLastError()
            )
            Objdll.NET_DVR_Logout(lUserId)
            Objdll.NET_DVR_Cleanup()
